...
use constant opt_param_tab => { 
    ...
    'IS95A_AlertOnDelivery' =>  0x1304, 
    'IS95A_LanguageIndicator' => 0x1306, 
    ...
    'mcc' => 0x1450, 
    'mnc' => 0x1451, 
    'PDC_MessageClass' => 0x1101, 
    'PDC_PresentationOption' => 0x1102, 

    'PDC_AlertMechanism' => 0x1103, 
    'PDC_Teleservice' => 0x1104, 
    'PDC_MultiPartMessage' => 0x1105, 
    'PDC_PredefinedMsg' => 0x1106
}; 
...
sub get_body_str {
    ...
    my $mandatory_parameters = pack $template, map { $data_ref->{$_} } @{$attr_seq};
    my $optional_parameters = encode_optional_parameters ($data_ref); 

    return $mandatory_parameters if $optional_parameters eq ''; 
    return $mandatory_parameters . $optional_parameters; 
}

sub encode_optional_parameters { 
    my ($data_ref) = @_; 
    my $data = '';  

    foreach my $tag ( keys %{ $data_ref }) { 
        if ( defined ( opt_param_tab->{$tag})) { 
            my $val = $data_ref->{$tag}; 
            warn "Added to PDU as optional: ".$tag . "=> ".$val; 
            $data .= pack 'nna*', opt_param_tab->{$tag}, length($val), $val;
        }
    }
    return $data; 
}
